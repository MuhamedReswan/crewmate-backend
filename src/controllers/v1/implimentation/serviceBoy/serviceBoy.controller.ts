...
export default class ServiceBoyController implements IServiceBoyController {
  ...
  loadProfile = async (
    ...
    try {
      ...
      const serviceBoyProfile = await this._serviceBoyService.LoadProfile({
        _id,
      });
  ...
  retryServiceBoyVerfication = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
  ...
  loadServiceBoyById = async (
    ...
    try {
      ...
      const _id = new Types.ObjectId(id);
      const serviceBoy = await this._serviceBoyService.loadServiceBoyById({
        _id,
      });
