...
export default class VendorAuthController implements IVendorAuthController {
  constructor(@inject("IVendorAuthService") private _vendorAuthService: IVendorAuthService) {}
  ...
  verifyOTP = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const vendorData = await this._vendorAuthService.verifyOTP(email, otp, oldRefreshToken);
  ...
  forgotPassword = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      await this._vendorAuthService.resetPasswordLink(forgotToken, email, Role.VENDOR);
      res
        .status(HttpStatusCode.OK)
  ...
  setNewAccessToken = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this._vendorAuthService.setNewAccessToken(refreshToken);
  ...
  logout = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!refreshToken) {
        res
          .status(HttpStatusCode.BAD_REQUEST)
          .json(responseHandler(ResponseMessage.INVALID_REFRESH_TOKEN, HttpStatusCode.BAD_REQUEST));
        return;
      }
