...
import { IVendorService } from "../../../../services/v1/interfaces/vendor/IVendor.service";
...
import { IVendorController } from "../../interfaces/vendor/IVendor.controller";
  ...
  loadVendorProfile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res
        ...
        .json(
          responseHandler(ResponseMessage.LOAD_PROFILE_SUCCESS, HttpStatusCode.OK, vendorProfile)
        );
    } catch (error) {
    ...
    try {
      ...
      if (updateVendorProfile) {
        res
          .status(HttpStatusCode.OK)
          .json(
            responseHandler(ResponseMessage.PROFILE_UPDATED, HttpStatusCode.OK, updateVendorProfile)
          );
      } else {
        res
  ...
  loadVendorById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!id || !validateObjectId(id)) {
        res
          .status(HttpStatusCode.BAD_REQUEST)
          .json(responseHandler(ResponseMessage.INVALID_VENDOR_ID, HttpStatusCode.BAD_REQUEST));
        return;
      }
      ...
      res
        .status(HttpStatusCode.OK)
        .json(responseHandler(ResponseMessage.UPDATE_STATUS_SUCCESS, HttpStatusCode.OK, vendor));
    } catch (error) {
      logger.error("VendorController: loadProfile error", error);
      next(error);
    }
