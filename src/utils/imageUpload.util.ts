...
export const resizeAndAssignBuffer = async (imageArray?: { buffer: Buffer }[]) => {
...
export const processAndUploadImage = async (
  ...
  if (imageArray && imageArray.length === 1) {
    ...
    const imageName = `${userName}-${imageType}-${Date.now()}`;
    await s3Util.uploadImageToBucket(imageArray[0].buffer, imageArray[0].mimetype, imageName);
    return imageName;
  }
  return undefined;
};
...
export const processAndUploadImagecloudinary = async (
  ...
  if (imageArray && imageArray.length === 1) {
    ...
    const result = await uploadImageToCloudinary(imageArray[0].buffer, {
      folderName,
      publicId: imageName,
      isSecure,
    });
